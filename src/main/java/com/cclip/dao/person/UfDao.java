...
public class UfDao {
	...
	public Uf findById(ConnectionWrapper connectionWrapper, String id) {

		Uf r = (Uf) utilJdbc.findById(connectionWrapper, "cclip.uf", id, new MapperResultFind());
	...
	public Uf[] find(ConnectionWrapper connectionWrapper) {

		return find(connectionWrapper, new MapperQueryOrderArg[0]);

	}

	public Uf[] find(ConnectionWrapper connectionWrapper, MapperQueryOrderArg[] orderList) {

		Object[] r = utilJdbc.find(connectionWrapper, "cclip.uf", new MapperResultFind(), orderList);

		Uf[] resultList = new Uf[r.length];
	...
	public Uf[] find(ConnectionWrapper connectionWrapper, MapperQueryArg[] argList, MapperQueryOrderArg[] orderList) {

		Object[] r = utilJdbc.findByExample(connectionWrapper, "cclip.uf", argList, new MapperResultFind(), orderList);
	...
	public ResultList find(ConnectionWrapper connectionWrapper, MapperQueryArg[] argList, MapperQueryOrderArg[] orderList, Integer offSet, Integer limit) {
		...
		ResultList r = utilJdbc.findByExamplePageable(connectionWrapper, "cclip.uf", argList, new MapperResultFind(), offSet, limit, orderList);
